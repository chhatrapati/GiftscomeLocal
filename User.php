...
class Cl_User
{
	...
	public function registration( array $data )
	{
		if( !empty( $data ) ){
			...
			$this->set_user_points($last_id,'20','By Register');
	...
	public function login( array $data )
	{
		...
		if( !empty( $data ) ){
			...
			if( $count >= 1){
				...
				$this->coins_by_login();
				$this->set_user_points($uid,'10','By Login');
	...
	public function fb_login( array $data )
	{
		...
		if( $count >= 1){
			...
			$this->set_user_points($uid,'10','By Social Login');
			$this->set_points_users($uid);
			return true;
		}else{
	...
	public function get_level_point($id){
		$sql = mysqli_query($this->_con,"select @temp_total := user_points as total_points,
			(case
				...
				when @temp_total > 200 and @temp_total < 500 then 'Level 5'
				when @temp_total > 500 then 'Level5'	
				else 'NO Points' 
					END) as 'user_level',
				(case
