...
{
$uid= $_SESSION['id'];
$user_type = $user_obj->user_type($uid);
...
$quer_987=mysqli_query($con,"select count(*) as updatetocurrent from tbl_game WHERE (SELECT UTC_TIMESTAMP) BETWEEN game_start_time AND (game_start_time + INTERVAL 3 HOUR_MINUTE) AND game_status='0'");
$row_987=mysqli_fetch_array($quer_987);
//print_r($row_987);
if($row_987['updatetocurrent'] > 0){	
	$sql_987=mysqli_query($con, "update tbl_game set game_status = '1' WHERE (SELECT UTC_TIMESTAMP) BETWEEN game_start_time AND (game_start_time + INTERVAL 3 HOUR_MINUTE) AND game_status='0'");	
}
?>
<!-- Header -->
	...
	<div class="container-menu-header">
		<div class="topbar">
			...
			<div class="wrap_header">
				...
				<div class="wrap_menu">
					<nav class="menu">
						<ul class="main_menu">
							...
							<li class="">
								<a href="view_friends.php"  onclick="localStorage.clear();">FRIENDS </a>
							</li>
							<li class="">
				...
				<div class="header-icons">
                    ...
                    <div class="header-wrapicon2">
                    	<span class="header-icons-noti badge">
						<?php
						 $items_list =$user_obj->cart_items($uid);
		                 $count = mysqli_num_rows($items_list);//print_r($items_list);
						if (!empty($count)) { echo  $count; } else { echo '0';} ?></span>
                    	<img src="images/icons/cart.png" class="header-icon1 js-show-header-dropdown" alt="ICON">
                    	<div class="header-cart header-dropdown order_table" id="">
                    		<?php
                    		if (!empty($count)) {
                    			$total = 0;
                    					...
                    					<li class="header-cart-item">
                    						...
                    						<div class="header-cart-item-txt">
                    							...
                    							<span class="header-cart-item-info">
                    								<?php echo number_format($price); ?> x <?php echo $values["product_quantity"]; ?>
                    							</span>
            ...
            <div class="wrap_header_mobile">
            	...
            	<div class="btn-show-menu">
					...
					<div class="header-wrapicon2">
						...
						<span class="header-icons-noti badge">
						<?php
						 $items_list1 =$user_obj->cart_items($uid);
		                 $count1 = mysqli_num_rows($items_list);
						if (!empty($count1)) { echo  $count1; } else { echo '0';} ?></span>
						<!-- Header cart noti -->
						<div class="header-cart header-dropdown order_table">
							<?php
							if (!empty($count1)) {
                    			$total1 = 0;
                    			while($values1 = mysqli_fetch_array($items_list1, MYSQLI_BOTH)) {
									$price= $values1["product_price"];
									$price = $price*$gift_coins_exchange_rate;
									if($user_type=='vip')
									{
									$act_productPrice=$price;
									...
									<ul class="header-cart-wrapitem">
										<li class="header-cart-item">
											<div class="header-cart-item-img">
												<img src="admin/productimages/<?php echo $values1["product_name"]?>/<?php echo $values1["product_image"];?>" alt="IMG">
											</div>

											<div class="header-cart-item-txt">
												<a href="#" class="header-cart-item-name">
													<?php echo $values1["product_name"]; ?>
												</a>
												<span class="header-cart-item-info">
													<?php echo number_format($productPrice); ?> x <?php echo $values1["product_quantity"]; ?>
												</span>
									...
									<?php
									$total1 = $total + ($values1["product_quantity"] * $productPrice);
									$_SESSION['tp'] = $total1;
								} ?>

								<div class="header-cart-total">
									Total: <?php echo number_format($total1); ?>
								</div>
			...
			<?php if(strlen(@$_SESSION['login'])==0) {   ?>
					...
					<div class="topbar-child2">
						<span class="topbar-email" style="padding-right:10px;">
							Welcome, <?php echo htmlentities(@$_SESSION['username_1']);?> <span class="badge-pill badge-success user-level"><?php echo $level_of_user;?> <?php echo $sub_lev;?> </span> <span style="margin-left:3px;" class="badge-pill badge-success <?php echo $cls;?>"><?php echo $user_type;?></span>
						</span>
						<span class="topbar-email"  style="padding-right:10px;">
							<a href="my-account.php"><i class="icon fa fa-user"></i> My account</a> <a href="support_us.php"><i class="icon fa fa-envelope"></i></a><span class="header-count"><?php echo $unread_msgs; ?> </span>
						</span>
