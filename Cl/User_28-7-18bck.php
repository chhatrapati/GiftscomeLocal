...
class Cl_User
{
	...
	public function registration( array $data )
	{
		if( !empty( $data ) ){
			...
			$query = "INSERT INTO users (id, name, username, nick_name, email, password, contactno, gender, shippingAddress, user_picture, regDate) VALUES (NULL, '$name', '$new_nick_name', '$new_nick_name', '$email', '$password', '$contact_no', '$gender', '$address', '$file', CURRENT_TIMESTAMP)";
	...
	public function fb_login( array $data )
	{
		...
		if( $count >= 1){
			...
			$query = "INSERT INTO users (id, password, name,username, email, social_id, user_picture, regDate) VALUES (NULL, '$password1', '$name','$name', '$email', '$social_id', '$fb_image', CURRENT_TIMESTAMP)";
			$query2 = mysqli_query($this->_con, "update `users` set online = 1 where id = '".$uid."'");
            $res = mysqli_query($this->_con, "UPDATE `users` SET online=1, last_active_timestamp = NOW() WHERE id = {$uid};");
			if(mysqli_query($this->_con, $query)){
				$to = $email;
	...
	public function google_login( array $data )
	{
		...
		if( $count == 1){
			...
			$password = $this->randomPassword();
			$password1 = md5( $password );			
			$query = "INSERT INTO users (id, password, name,username, email, social_id, user_picture, regDate) VALUES (NULL, '$password1', '$name', '$name', '$email', '$social_id', '$picture', CURRENT_TIMESTAMP)";
			$query2 = mysqli_query($this->_con, "update `users` set online = 1 where id = '".$uid."'");
            $res = mysqli_query($this->_con, "UPDATE `users` SET online=1, last_active_timestamp = NOW() WHERE id = {$uid};");
			if(mysqli_query($this->_con, $query)){
				$to = $email;
