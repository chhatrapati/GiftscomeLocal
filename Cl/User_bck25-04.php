...
class Cl_User
{
	...
	public function all_games(){
				$query=mysqli_query($this->_con, "select * from tbl_game  WHERE is_active=1 order by id desc ");
		        return $query;
		
	}
	/*Code of fetch total bids on given  no of game*/
	public function users_total_bids_on_no($game_id,$digit,$user_id){
				$sql = mysqli_query($this->_con,"select SUM(bid_amount) FROM tbl_userbids WHERE game_id ='$game_id' AND bid_no ='$digit' AND user_id ='$user_id' ");
	...
	public function last_bet_by_user($game_id,$user_id){
		        //echo $game_id;
				//echo $user_id;
				$sql = mysqli_query($this->_con,"select bid_no,bid_amount FROM tbl_userbids WHERE game_id ='$game_id' AND user_id='104'");
				return $sql;
			
	}
	/*Code of fetch total User level*/
	public function User_level($id){
		$sql = mysqli_query($this->_con,"select @temp_total := (refferal + buy_coin + daily_login + social_share) as total_points,
			(case
				...
				when @temp_total > 200 then 'Level5'	
				else 'NO Points' 
					END) as 'user_level',
				(case
					...
					when @temp_total > 200 then 0
					else 'NO Points' 
						END) as 'next_level'
					from user_point where user_id ='$id';");
				$result=mysqli_fetch_array($sql);//print_r($result44);
				return $result;
			}	
	  /*Get Most_popular users list by added new*/
	 public function get_Popular_user(){
		$query12334 = mysqli_query($this->_con,"select * FROM users order by game_coins desc limit 12");		
		return $query12334;
