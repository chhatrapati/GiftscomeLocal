...
class Cl_User
{
	...
	public function registration( array $data )
	{
		if( !empty( $data ) ){
			...
			$reg_by_ip = $this->get_client_ip_env();
			$result_921 = mysqli_query($this->_con,"SELECT * FROM users where reg_by_ip = '$reg_by_ip' and refer_by = '$user_id_by_ref' ");
			$count_ip_same = mysqli_num_rows($result_921);
			}
			else
			{
			$user_id_by_ref='';
			...
			if($ref_by_code !='' && $count_ip_same==0)
			{
			$query = "INSERT INTO users (id, name, username, nick_name, email, password, contactno, gender, shippingAddress, user_picture,referal_code, refer_by,regDate,reg_by_ip) VALUES (NULL, '$name', '$new_nick_name', '$new_nick_name', '$email', '$password', '$contact_no', '$gender', '$address', '$file','$referal_code', '$user_id_by_ref', CURRENT_TIMESTAMP,'$reg_by_ip')";
			}
			if($ref_by_code !='' && $count_ip_same>=1)
			{
			...
			$this->set_points_users($last_id,$points_by_register);
			...
			if($ref_by_code !='')
			{
				$this->coins_using_refercode_reg($last_id,$ref_by_code);
				$this->coins_by_refer_friend($user_id_by_ref,$last_id);
			}
            return true;
		} else{
			throw new Exception( USER_REGISTRATION_FAIL );
	...
	public function login( array $data )
	{
		...
		if( !empty( $data ) ){
			...
			if( $count >= 1){
				...
				$date_of_points_get = $this->points_get_date($uid,'By Login');
	...
	public function coins_by_login() {
		...
		//$ss=  mysqli_query($this->_con,"SELECT COUNT(1) FROM userlog  WHERE loginTime >= DATE_SUB(NOW(), INTERVAL 12 HOUR) AND user_id='$id'");
		$ss=  mysqli_query($this->_con,"SELECT logindate FROM userlog  WHERE user_id='$id' order by loginTime desc limit 1");
		$result_data = mysqli_fetch_assoc($ss);
		$logindate = $result_data['logindate'];
		$cur_date= date("Y-m-d");
		//$no_of_login = $result_data['COUNT(1)'];
		//echo '<pre>';print_r($result_data);if($cur_date>$logindate){echo 'greater';} else {echo 'less';}die();
				...
				$sql12=  mysqli_query($this->_con,"select * from coins_transfer_value where user_type='$user_type'")or die(mysqli_error());
				$data123=mysqli_fetch_array($sql12,MYSQLI_ASSOC); //echo '<pre>';print_r($data123); 
				$daily_gift_coins = $data123['gift_coins'];
				if($cur_date > $logindate)
				{ 
					/*Insert coins into user wallet*/
		            $insert_users_coins_in_wallet = $this->inser_user_coins_in_wallet($id,$daily_gift_coins,$remarks);
				    /*Update users default balance*/
					$update_users_coins =$this->update_user_coins_final_balance($id,$daily_gift_coins);
	...
	/*Code of coins transfer to user on facebook share by day*/
	...
	/*Code of coins transfer to user refer to friend and register by referal code*/
	public function coins_value_by_refer_code() {
		        $sql=  mysqli_query($this->_con,"select coins_by_refer_code from coins_reward_method")or die(mysqli_error());
				$result=mysqli_fetch_array($sql);
				$gift_coins =$result['coins_by_refer_code'];
				return $gift_coins;
	...
	public function coins_by_register($last_id) {
		...
		@$id =$last_id;
		$sel=  mysqli_query($this->_con,"select coins_by_register from coins_reward_method")or die(mysqli_error());
				$res=mysqli_fetch_array($sel,MYSQLI_ASSOC); //echo '<pre>';print_r($data1); die();
				//$game_coins = $res['game_coins_on_register'];
				$gift_coins = $res['coins_by_register'];
				$remarks ='By Register';
				/*Insert coins into user wallet*/
		        $insert_users_coins_in_wallet = $this->inser_user_coins_in_wallet($id,$gift_coins,$remarks);
				/*Update users default balance*/
	...
	public function coins_using_refercode_reg($last_id,$ref_by_code) {
		        @$id =$last_id;
				$gift_coins = $this->coins_value_by_refer_code();
				$remarks ='By Register Using Referal Code '.$ref_by_code;
				/*Insert coins into user wallet*/
		        $insert_users_coins_in_wallet = $this->inser_user_coins_in_wallet($id,$gift_coins,$remarks);
				/*Update users default balance*/
				$update_users_coins = $this->update_user_coins_final_balance($id,$gift_coins);
   ...
   /*Code of coins transfer to user by referal code share to friend*/
	public function coins_by_refer_friend($last_id,$new_user_id) {
		        @$id =$last_id;
				$gift_coins = $this->coins_value_by_refer_code();
				$query_new=  mysqli_query($this->_con,"select * from users where id='$new_user_id'")or die(mysqli_error());
				$data_new=mysqli_fetch_array($query_new,MYSQLI_ASSOC);
				$uname = $data_new['name'];
				$remarks ='By Refer To Friend '.$uname;
				/*Insert coins into user wallet*/
		        $insert_users_coins_in_wallet = $this->inser_user_coins_in_wallet($id,$gift_coins,$remarks);
				/*Update users default balance*/
				$update_users_coins = $this->update_user_coins_final_balance($id,$gift_coins);
...
/*Code of gift and game coins update on users table*/
	...
	/*Code of gift and game coins update on users table*/
	public function inser_user_coins_in_wallet($id,$gift_coins,$remarks){
		            $query_142=mysqli_query($this->_con, "insert into user_wallet(user_id,user_coins,reason_mode_of_coins,coins_type,date_of_coins_get) values('".$id."','$gift_coins','$remarks','gift_coins',NOW())");
	}
	...
	public function get_winners($limit){
		        $sql788=mysqli_query($this->_con, "select user_id,SUM(total_bid_amount) as total_bid_amount,game_id,create_date,SUM(total_won_coins) as total_won from tbl_winners group by user_id order by total_won desc  limit $limit");
				return $sql788;
    ...
    return $query12333;
	 ...
	 /*Code of fetch user details by user id*/
		...
		public function times_of_coins_get($id){
					$sql = mysqli_query($this->_con,"SELECT COUNT(1) FROM user_wallet  WHERE reason_mode_of_coins='Coins Supplement On Request To Admin' AND date_of_coins_get = CURRENT_DATE() AND user_id='$id'");
					$result=mysqli_fetch_array($sql); //print_r($result);
					return $result;
	...
	/*public function get_level_point($id){
		$sql = mysqli_query($this->_con,"select @temp_total := user_points as total_points,
			(case
				when @temp_total >= 0 and @temp_total <= 20 then 'Gold'
				when @temp_total > 20 and @temp_total <= 50 then 'Silver'
				when @temp_total > 50 and @temp_total <= 100 then 'Platinum'
				when @temp_total > 100 and @temp_total <= 200 then 'Diamond'
				when @temp_total > 200 then 'God'
				else 'NO Points' 
					END) as 'user_level',
				(case
	...
	public function get_level_point($id){
		$sql = mysqli_query($this->_con,"select @temp_total := u.user_points,
        ...
        @user_levels_complete5:= (select  user_levels_complete from user_levels where user_levels_id = 5),
            ...
            (case
                when u.user_points >= @user_levels_complete1 and u.user_points < @user_levels_complete2 then @user_levels_name1 ELSE CASE 
             when u.user_points >= @user_levels_complete2 and u.user_points < @user_levels_complete3 then @user_levels_name2  ELSE CASE 
              when u.user_points >= @user_levels_complete3 and u.user_points < @user_levels_complete4 then @user_levels_name3  ELSE CASE 
			   when u.user_points >= @user_levels_complete4 and u.user_points < @user_levels_complete5 then @user_levels_name4  ELSE CASE
             when u.user_points >= @user_levels_complete5 then @user_levels_name5
	...
	public function get_all_level_points($id){
		$sql = mysqli_query($this->_con,"select @temp_total := u.user_points,
		(select @user_levels_complete1:= user_levels_complete from user_levels where user_levels_id = 1) as user_levels_complete1,
        (select @user_levels_complete2:= user_levels_complete from user_levels where user_levels_id = 2) as user_levels_complete2,
        (select @user_levels_complete3:= user_levels_complete from user_levels where user_levels_id = 3) as user_levels_complete3,
        (select @user_levels_complete4:= user_levels_complete from user_levels where user_levels_id = 4) as user_levels_complete4,
        (select @user_levels_complete5:= user_levels_complete from user_levels where user_levels_id = 5) as user_levels_complete5
		from users as u where id ='$id';");
		$result=mysqli_fetch_array($sql);
		return $result;
...
// By share on facebook
	...
	/*public function latest_games(){
		       $query=mysqli_query($this->_con, "select * from tbl_game WHERE is_active=1 and game_start_time BETWEEN  (UTC_TIMESTAMP - INTERVAL 45 HOUR_MINUTE) AND (UTC_TIMESTAMP + INTERVAL 15 HOUR_MINUTE) order by id desc");
		        return $query;
		
	}*/
public function latest_games($uid){
		       ...
		       $query=mysqli_query($this->_con, "select id,game_name,game_start_time,game_duration,game_status,winning_no,CONCAT(game_wining_number1 ,'+' , game_wining_number2 , '+' ,game_wining_number3 ,'=' , winning_no) as winno,
(select SUM(bid_amount) FROM tbl_userbids WHERE game_id =tbl_game.id) as total_bids_on_game,
(select SUM(bid_amount) FROM tbl_userbids WHERE game_id =tbl_game.id AND user_id ='$uid') as total_my_bet,
(select payout_amount FROM tbl_game_payout WHERE game_id =tbl_game.id AND payout_digit = tbl_game.winning_no)as payout_amount_onwin_no,
(select SUM(bid_amount) FROM tbl_userbids WHERE game_id =tbl_game.id AND bid_no =tbl_game.winning_no) as total_bids_byall_onwin_no,
(select SUM(bid_amount) FROM tbl_userbids WHERE game_id =tbl_game.id AND bid_no =tbl_game.winning_no AND user_id ='$uid') as total_bids_onwin_no
from tbl_game
WHERE is_active=1 and game_start_time BETWEEN  (UTC_TIMESTAMP - INTERVAL 45 HOUR_MINUTE) AND (UTC_TIMESTAMP + INTERVAL 15 HOUR_MINUTE) order by id desc");
		       return $query;
	...
	/*get gift_coins_exchange_rate*/
	public function gift_coins_exchange_rate(){
				$sql = mysqli_query($this->_con,"select gift_exchange_rate FROM general_settings");
				$result=mysqli_fetch_array($sql);
				$gift_exchange_rate =$result['gift_exchange_rate'];
				return $gift_exchange_rate;
			
	}
	/*Date of points get by user id and by remarks*/
	public function points_get_date($uid,$points_by){
				$sql = mysqli_query($this->_con,"SELECT date_of_points_get FROM tbl_users_points  WHERE user_id='$uid' AND points_by='$points_by' order by date_of_points_get desc limit 1");
				$result=mysqli_fetch_array($sql);
				$date_of_points_get =$result['date_of_points_get'];
				return $date_of_points_get;
...
/*Total user points by user id*/
	public function user_total_points($uid){
				$sql = mysqli_query($this->_con,"SELECT user_points FROM users WHERE id = '$uid'");
				$result=mysqli_fetch_array($sql);
	...
	/*Code of coins transfer to normal user by request to daily login reward*/
	public function coins_by_daily_login_request() {
		
		@$id =$_SESSION['id'];		
		$all_data =$this->get_user_coins_balance($id); /*Get users default balance*/
		$gift_coins = $all_data['gift_coins'];
		$remarks ='By Redeem Daily Login Giftcoins';
		$sql12=  mysqli_query($this->_con,"select * from coins_transfer_value where user_type='normal'")or die(mysqli_error());
		$data123=mysqli_fetch_array($sql12,MYSQLI_ASSOC); //echo '<pre>';print_r($data123); 
		$daily_gift_coins = $data123['gift_coins'];
		/*Insert coins into user wallet*/
		$insert_users_coins_in_wallet = $this->inser_user_coins_in_wallet($id,$daily_gift_coins,$remarks);
	...
	/*Code to fetch general settings data*/
	public function get_general_setting(){
				$sql = mysqli_query($this->_con,"select * FROM general_settings");
				$result=mysqli_fetch_array($sql);
				return $result;
			
	}
	
	/*Code to fetch unread ticket message of admin to user */
	public function count_unread_msg($uid){
				$sql = mysqli_query($this->_con,"SELECT ticket_id FROM comments where user_id = '$uid'");
				$total = 0;
				while($result=mysqli_fetch_array($sql))
				{
					$ticket_id = $result['ticket_id'];
					$result_query = mysqli_query($this->_con, "SELECT count(*) as total FROM comments WHERE status_by_user=0 and ticket_id = '$ticket_id'");
					$row12= mysqli_fetch_array($result_query);
					$pp =$row12['total'];
					$total += $pp;
				}
				return $total;
	...
	public function cart_items($user_id){
		//$sql=  mysqli_query($this->_con,"SELECT * FROM tbl_cart  WHERE user_id='$user_id' AND is_purchased='0' ");
		$sql=  mysqli_query($this->_con,"select product_id,product_quantity,is_purchased,
(select productName FROM products WHERE id =tbl_cart.product_id) as product_name,
(select productPrice FROM products WHERE id =tbl_cart.product_id) as product_price,
(select productImage1 FROM products WHERE id =tbl_cart.product_id) as product_image,
(select gift_coins_value FROM products WHERE id =tbl_cart.product_id) as gift_coins_value,
(select category FROM products WHERE id =tbl_cart.product_id) as product_cat,
(select validity_of_vip_package FROM products WHERE id =tbl_cart.product_id) as validity_of_vip_package
from tbl_cart WHERE user_id='$user_id' AND is_purchased='0' ");
		return $sql;
		
	}
	
	/*get quantity of product if already added to cart*/
	public function get_product_quantity($user_id,$product_id){
		$sql=  mysqli_query($this->_con,"SELECT product_quantity FROM tbl_cart  WHERE user_id='$user_id' AND is_purchased='0' AND product_id ='$product_id'");
		$result_data = mysqli_fetch_array($sql);
	    $product_quantity = $result_data['product_quantity'];
		return $product_quantity;
		
	}
	
	/*insert product into cart table*/
	public function insert_pro_cart($user_id,$product_id,$product_quantity){
		$sql=  mysqli_query($this->_con,"insert into tbl_cart(user_id,product_id,product_quantity) values('$user_id','$product_id','$product_quantity')");
		return $sql;
		
	}
	/*Update product into cart table if already added into cart*/
	public function update_pro_cart($user_id,$product_id,$product_quantity){
		$sql=  mysqli_query($this->_con,"UPDATE tbl_cart SET product_quantity = '$product_quantity' WHERE user_id='$user_id' AND is_purchased='0' AND product_id ='$product_id'");
		return $sql;
		
	}
	/*Remove product from cart table*/
	public function remove_pro_cart($user_id,$product_id){
		$sql=  mysqli_query($this->_con,"delete from tbl_cart WHERE user_id='$user_id' AND is_purchased='0' AND product_id ='$product_id'");
		return $sql;
		
	}
	/*Update product status into cart table after payment done*/
	public function update_pro_payment_status($user_id,$product_id){
		$sql=  mysqli_query($this->_con,"UPDATE tbl_cart SET is_purchased = '1' WHERE user_id='$user_id' AND product_id ='$product_id'");
		return $sql;
