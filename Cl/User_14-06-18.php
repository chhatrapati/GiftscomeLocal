...
class Cl_User
{
	...
	public function get_game_history(){
				$query_new=mysqli_query($this->_con, "select * from tbl_game  WHERE game_status ='3' order by id desc");
		        return $query_new;
...
// By share on facebook
	...
	public function latest_games(){
		       $query=mysqli_query($this->_con, "select * from tbl_game WHERE is_active=1 and game_start_time BETWEEN  (UTC_TIMESTAMP - INTERVAL 15 HOUR_MINUTE) AND (UTC_TIMESTAMP + INTERVAL 15 HOUR_MINUTE) order by id desc");
		        return $query;
