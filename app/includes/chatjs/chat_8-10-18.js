...
var originalTitle;
var blinkOrder = 0;
var audioogg = new Audio(siteurl+'app/includes/audio/chat.ogg');
var audiomp3 = new Audio(siteurl+'app/includes/audio/chat.mp3');
...
emojione.imagePathPNG = 'app/plugins/smiley/assets/png/';
...
function msg_eventpl(chatboxtitle,sender,img,message_content,time,position){
    //var tooltip = '<div class=\'tooltiptext\'><img class=\'pull-left\' src=\'users-images/'+img+'\'> '+sender+' <i>'+time+'</i></div>';
...
function restructureChatBoxes() {
    ...
    for (x in chatBoxes) {
        ...
        if ($("#chatbox_"+chatboxtitle).css('display') != 'none') {
            if (align == 0) {
                $("#chatbox_"+chatboxtitle).css('right', '325px');
            } else {
                width = (align)*(273+7)+325;
...
function createChatBox(chatboxtitle,chatheaderimg,status,minimizeChatBox) {
    ...
    $("<div />" ).attr("id","chatbox_"+chatboxtitle)
		...
		.css("z-index", "10000")
        .html('<div class="chatbox-icon" onclick="javascript:toggleChatBoxGrowth(\''+chatboxtitle+'\')" href="javascript:void(0)"><div class="contact-floating red"><img class="chat-image img-circle pull-left" src="users-images/'+chatheaderimg+'"><small class="unread-msg">2</small><small class="status Online"></small></div></div>' +
        '<div class="panel personal-chat"> ' +
        '<div class="panel-heading chatboxhead"> ' +
        '<div class="panel-title">' +
        '<img class="chat-image img-circle pull-left" height="36" width="36" src="users-images/'+chatheaderimg+'" alt="avatar-image"> ' +
        '<div class="header-elements">' +
        '<a href="user-profile.php?uname='+chatboxtitle+'">'+chatboxtitle+'</a>' +
        '</h3>' +
        '<br> ' +
        '<small class="status '+status+'"><b>'+status+'</b>' +
        '</small> ' +
        ...
        '<div class="input-placeholder" style="visibility: hidden; display: none;">Type a message</div> ' +
        '<textarea class="input chatboxtextarea" id="chatboxtextarea" name="chattxt" contenteditable="" spellcheck="true" style="resize:none;height:20px" placeholder="Type a message" onkeydown="javascript:return checkChatBoxInputKey(event,this,\''+chatboxtitle+'\');"></textarea>' +
    ...
    if (chatBoxeslength == 0) {
        ...
        width = (chatBoxeslength)*(273+7)+325;
        $("#chatbox_"+chatboxtitle).css('right', width+'px');
    }
...
function chatHeartbeat(){
    ...
    $.ajax({
        url: siteurl+"chat.php?action=chatheartbeat",
        cache: false,
        dataType: "json",
...
function get_all_msg(url){
    ...
    $.ajax({
        //url: siteurl+"chat.php?action=get_all_msg&client="+client,
        url: url,
        cache: false,
        dataType: "json",
        success: function(data) {
...
function checkChatBoxInputKey(event,chatboxtextarea,chatboxtitle) {
    ...
    if(event.keyCode == 13 && event.shiftKey == 0)  {
        ...
        if (message != '') {
            $.post(siteurl+"chat.php?action=sendchat", {to: chatboxtitle, message: message} , function(data){
                ...
                msg_oddtpl(chatboxtitle,session_uname,session_img,message,"Just Now","append");
...
function lastseen(uname){
    $.ajax({
        url: siteurl + "chat.php?action=lastseen&uname="+uname,
        cache: false,
        type: "POST",
        success: function (data) {
            if(data == "Online"){
            ...
            $.each(data.items, function(i,item){
                if (item)	{ // fix strange ie bug
                    ...
                    if (item.s == 2) {
                        ...
                        if (item.u == 2) {
                            //$("#chatbox_"+chatboxtitle+" .chatboxcontent").append('<div class="chatboxmessage direct-chat-msg"><div class="direct-chat-info clearfix"><span class="direct-chat-name pull-left">'+item.f+'</span></div><img class="direct-chat-img" src="users-images/'+item.p+'" alt="message user image"><span class="direct-chat-text">'+item.m+'</span></div>');
                        } else {
                            //$("#chatbox_"+chatboxtitle+" .chatboxcontent").append('<div class="chatboxmessage direct-chat-msg right"><div class="direct-chat-info clearfix"><span class="direct-chat-name pull-right">'+item.f+'</span></div><img class="direct-chat-img" src="users-images/'+img+'" alt="message user image"><span class="direct-chat-text">'+item.m+'</span></div>');
                        }
                    }
                }
            });
...
function closeChatBox(chatboxtitle) {
    ...
    $.post(siteurl+"chat.php?action=closechat", { chatbox: chatboxtitle} , function(data){
    });
...
function refreshTypingStatus(chatboxtitle){
    if (!$('#textarea').is(':focus') || $('#textarea').val() == '' || new Date().getTime() - lastTypedTime.getTime() > typingDelayMillis) {
        ...
        $.post(siteurl+"chat.php?action=typingstatus", {to: chatboxtitle, typing: 1} , function(data){
...
function uploadimage(touname) {
    ...
    var base_url = 'process.php?tun='+tun+'&fun='+fun;
    var file_data=$("#chatbox_"+touname+" #imageInput").prop("files")[0];
    var form_data=new FormData();
    form_data.append("file",file_data);

    $('#loadmsg_'+touname).show();      // Loader show
        ...
        success:function(data){
            $.each(data.items, function(i,item){
                if (item)	{ // fix strange ie bug
                    ...
                    sender = item.sender;
                    msg_oddtpl(chatboxtitle,sender,session_img,message_content,"11.20 pm","append");
                }
            ...
            plupload.each(arg, function(value, key) {
                if (typeof(value) != "function") {
                    if(key == "response"){
                        ...
                        msg_oddtpl(toName,session_uname,session_img,message_content,"Just Now","append");
