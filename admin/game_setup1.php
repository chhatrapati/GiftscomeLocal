...
if(isset($_POST['submit']) && @$_SESSION["csrf_token"] == @$_POST['csrf_token'])
{
	...
	$sql12=mysqli_query($con,"insert into tbl_game(game_name,game_start_time,game_duration,game_status) values('$game_name','$game_start_time','$game_duration',0)");
...
$time_extend = date('Y-m-d H:i:s',strtotime('+2 minutes',strtotime($currentTime)));
	
	$game_name=$_POST['game_name'];
	$game_start_time=$time_extend;
	$game_duration='3';
	$sql=mysqli_query($con, "insert into tbl_game(game_name,game_start_time,game_duration,game_status) values('$game_name','$game_start_time','$game_duration',0)");
	$lastid = mysqli_insert_id($con);
	/*Set payout rate of new create game in table tbl_game_payout*/
	$sql23=mysqli_query($con, "select * from tbl_default_payout");
	while($result=mysqli_fetch_array($sql23)) {
   ...
   <div class="row-fluid">
    <div class="span11">
	  ...
	  <?php if(!empty($_SESSION['msg'])){?>
		...
		<div class="controls">
	    <input size="16" type="text" name="game_name" id="game_name" class="" value="<?php echo $game_name.@$id;?>" placeholder="Enter Game Name">
	    </div>
	    </div>
   ...
   <div class="row-fluid">
    <div class="span11">
	  ...
	  <?php if(!empty($_SESSION['msg'])){?>
		...
		<div class="controls">
	    <input size="16" type="text" value="" name="game_start_time" id="game_start_time" readonly class="form_datetime" placeholder="Select Game Starting Time" required>
	    </div>
	    </div>	
