<?php
$userid =$_SESSION['id'];
$pre_bids = mysqli_query($con,"SELECT * FROM tbl_user_robot where user_id='$userid' AND status=1");
$resdata=mysqli_fetch_array($pre_bids);
	...
	$no_of_games = $resdata['no_of_games'];
    $game_id = $resdata['game_id'];
	$total_pre_bids = $resdata['total_bids'];
$ret = mysqli_query($con,"select * from tbl_game WHERE id >='$game_id' AND game_status=0 limit $no_of_games");
//print_r($ret);die();
