...
<body class="animsition">
          ...
          <div class="row" id="productContainer">
            <?php
			  ...
			  if (isset($_GET['pageno'])) {
				...
				$total_pages_sql = "SELECT COUNT(*) FROM users where id not in (".implode(",",$newarray).")";
				$res = mysqli_query($con,$total_pages_sql);
				$total_rows = mysqli_fetch_array($res)[0];
				$total_pages = ceil($total_rows / $no_of_records_per_page);
			  
			  
			  $query = "SELECT * FROM  users where id not in (".implode(",",$newarray).") LIMIT $offset, $no_of_records_per_page";
 ...
 <!-- chat script template for friends section due to conflict issue -->
<?php require_once('templates/chat_script_for_friendspage.php');?>
</body>
</html>