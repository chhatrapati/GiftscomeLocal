...
header('location:login.php');
	...
	foreach ($_SESSION["shopping_cart"] as $keys => $values)
	{
		...
		{
			$sql123 =mysqli_query($con,"SELECT expire_date FROM package_sales WHERE user_id='$uid' order BY expire_date DESC limit 1");
			...
			$package_validity =$values["product_validity_of_vip_package"];
			$package_validity= $package_validity*$pro_quantity;
			$gift_coins_by_pckg =$values["product_gift_coins"];
